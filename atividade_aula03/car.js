...
function mainCar() {
    ...
    // ---- DESENHAR CHASSI (Retângulo Cinza) ----
    // Para o chassi, um trapézio com larguras iguais é um retângulo.
    ...
    /* 
    // ---- DESENHAR CABINE MAIOR (Trapézio Vermelho) ----
        const cabineMaiorVertices = setTrapezoidVertices(-0.5, 0.1, 0.9, 0.6, 0.4); // baseWidth > topWidth
        const cabineMaiorColors = createSolidColor(6, [0.5, 0.5, 0.5]);
        gl.bindBuffer(gl.ARRAY_BUFFER, vertexBuffer);
        gl.bufferData(gl.ARRAY_BUFFER, cabineMaiorVertices, gl.STATIC_DRAW);
        gl.vertexAttribPointer(positionLocation, 2, gl.FLOAT, false, 0, 0);
        gl.bindBuffer(gl.ARRAY_BUFFER, colorBuffer);
        gl.bufferData(gl.ARRAY_BUFFER, cabineMaiorColors, gl.STATIC_DRAW);
        gl.vertexAttribPointer(colorLocation, 3, gl.FLOAT, false, 0, 0);
    
        gl.drawArrays(gl.TRIANGLES, 0, 6);  

    // ---- DESENHAR CABINE (Trapézio Azul) ----
    const cabineVertices = setTrapezoidVertices(-0.4, 0.1, 0.7, 0.5, 0.3); // baseWidth > topWidth
    const cabineColors = createSolidColor(6, [0.2, 0.5, 1.0]);
